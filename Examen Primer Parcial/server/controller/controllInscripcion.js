...
//ELIMIANR TODOS CON UN SOLO METODO
exports.delete = (req, res) => {
  const data = req.params.data;
  Inscripcion.deleteMany(data)
    .then((data) => {
      if (!data) {
        res
          .status(404)
          .send({
            message: `no se puede eliminar con id ${data}. tal vez la identificación está mal`,
          });
      } else {
        res.send({
          message: "La inscripcion fue eliminado exitosamente",
    ...
    .catch((err) => {
      res.status(500).send({
        message: "No se pudo eliminar el inscripcion con id=" + data
      });
    });
};
